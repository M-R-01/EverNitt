...
colors = [[5,107,0,19,255,255],
          ...
          [57,76,0,100,255,255],
          [90,48,0,118,255,255]]
myColors =[[51,153,255],[255,0,255],[0,255,0],[255,0,0]]
myPoints = []
def colorFinder(img, colors, myColors):
...
def draw(myPoints,myColors):
    for point in myPoints:
        cv2.circle(imgResult, (point[0],point[1]), 10, myColors[point[2]], cv2.FILLED)

while True:
    succ, img = vid.read()
    imgResult = img.copy()
    newPoints = colorFinder(img, colors, myColors)
    if len(newPoints) != 0:
